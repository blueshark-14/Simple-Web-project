<?php 
   ...
   if($_SERVER['REQUEST_METHOD'] == "POST")
   {
      ...
      if(!empty($company_name) && !empty($job_title) && !empty($address) && !empty($city) && !empty($country) && !empty($education) && !empty($publish_date) && !empty($deadline) )
      {
        //save to database
       // $id = random_num(20);
        $query = "insert into requirements( company_name, job_title, address , city , country, education , publish_date, deadline ) values( '$company_name' , '$job_title' , '$address', '$city', '$country', '$education', '$publish_date', '$deadline' ); ";  
